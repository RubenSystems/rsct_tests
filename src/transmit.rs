...
fn main() {
    ...
    let server = runtime.block_on(async {
        Server::<BasicAllocator>::new("0.0.0.0", "5253", BasicAllocator).await
    });
    ...
    let cli = rsct::client::Client::from_string("127.0.0.1:5254".to_string());

    let now = Instant::now();
    runtime.block_on(async {
        server.transmit_concurrently(string, &cli, &runtime).await;
    });
